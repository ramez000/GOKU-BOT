...
const ff = async (m, { text, conn }) => {
  ...
  if (!text) return m.reply("❌ : اكتب اسم الفيديو مع الأمر");
  ...
  try {
    ...
    const urlMatch = html.match(/https:\/\/www\.tiktok\.com\/@[^"']+\/video\/\d+/);

    if (!urlMatch) {
      return m.reply("لا توجد نتائج لهذا البحث: ❌");
    }

    const videoUrl = urlMatch[0];

    // =========================
    // ⬇ نفس سيرفر التحميل (ما تغيّر)
    // =========================
    const videoData = await downloadTikTok(videoUrl);

    if (!videoData.videoUrl && !videoData.audioUrl) {
      return m.reply("حدث خطأ في التحميل ❌");
    }

    if (videoData.videoUrl) {
      await conn.sendMessage(m.chat, {
        video: { url: videoData.videoUrl },
        caption: `🟢 ${videoData.description || "no description"}`
      });
    }
