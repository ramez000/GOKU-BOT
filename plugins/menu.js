...
menu.before = async (m, { conn, bot }) => {
    ...
    if (!cat) {
        await conn.sendMessage(m.chat, { text: '*❌≥ اختار رقم من القائمة فقط*' }, { quoted: reply_status });
        return true;
    }
    
    const cmds = menuData.cats[cat[2]];
    if (!cmds?.length) {
        await conn.sendMessage(m.chat, { text: '*❌≥ القسم فارغ*' }, { quoted: reply_status });
        return true;
    }
